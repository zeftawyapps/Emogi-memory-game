...
int _imagelevel = 0  ;
  void gameInit()async{
   ...
   int  colomesno =_getDatafromjson( stagesManager.colomesno);;


    gameConraller = GameConraller(gamelevle: _gamelevle??1, cardnum: _cardnum, imagelevel: _imagelevel, lastitemno: _lastitemno, colomesno: colomesno, myscors: 0, trayes: _settryies(), helpAddTryis: _helpAddTryis??20, helpAddCurrectCard: _helpAddCurrectCard??20);
  ...
  Future  _loadSavedData()async  {
    ...
    sharedPreferences = await SharedPreferences.getInstance();
      _gamelevle  =    sharedPreferences.getInt(sharedStages)??1;
    _helpAddTryis  =    sharedPreferences.getInt(sharedhelpadd)??20;
    _helpAddCurrectCard  =    sharedPreferences.getInt(sharedhelpcurrect)??20;
  }
  int _settryies(){
    if (_gamelevle < 50){return _cardnum*3 ; }
    if (_gamelevle > 50 && _gamelevle < 70  ){return _cardnum*3 - 5 ; }
